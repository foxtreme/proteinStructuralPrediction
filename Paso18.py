...
Lists the selected proteins to use in family classification
creates the training and testing datasets
to use in weka, WITH class and family verification
...
familyCompleteProteins = list()
#to quickly access the proteins and classifications
SCOP = dict()
Fquantification = dict() #keep track of proteins by superfamily
#name and class
#create lists with both+
contador = 0
familyZero = "a.1.1.0" #starting family

for x in lines:
    y = x.split()[0] 
    name = y[1:len(y)] 
    family = x.split()[1]#classification a.1.1.1
    family_only = family.split('.')[3]
    pair = list()
    pair.append(name)
    pair.append(family)
    SCOP[y] = family
    ...
    '''
    #creates a list of proteins by family
    
    if family == familyZero:
        contador = contador+1  #si es igual a la anterior incrementa la cuenta y pasa a la siguiente
    else:
        Fquantification[familyZero] = contador
        familyZero = family
        contador = 1
    
    familyComplete.append(family_only)
    familyCompleteProteins.append(pair)
    
selectedFamilies = list()
total = 0

#to select only families with enough data
print "family - quantity of proteins"
print "============ FILTERED =========="
for x in Fquantification.keys():
    ...
    if val >= 30:
        print "%s, %d"%(x,Fquantification[x])
        selectedFamilies.append(x)
   
print "-------- the families with more data ----------"
for x in selectedFamilies:
    print x
print "total of families %d -- average family: %d"%(len(selectedFamilies),total/len(Fquantification))

#header of the training file
f2 = file("sources/family_level/trainingDataset-family_U30.arff","w")
...
f2.write("@ATTRIBUTE class {b.34.2.1,b.34.2.0,c.3.1.5,d.108.1.1,a.104.1.0,c.67.1.0,c.93.1.0,c.94.1.1,c.94.1.0,c.2.1.0,c.2.1.3,c.2.1.2,d.15.1.1,b.36.1.0,b.36.1.1,d.144.1.0,d.144.1.7,b.40.4.5,c.1.11.0,c.37.1.19,c.14.1.0,c.66.1.0,c.108.1.0,c.47.1.0,a.121.1.1,c.23.1.0,d.58.7.0,d.58.7.1,b.1.2.1,b.55.1.1,g.37.1.1,c.37.1.8,b.1.1.1,c.69.1.0,a.4.5.0,a.4.1.9,g.39.1.3,c.37.1.0,c.37.1.1,b.1.1.0,b.1.1.4}\n")
f2.write("@DATA\n")

f3 = file("sources/family_level/selectedProteins-family_U30.txt","w")
#name of the protein in class %
#superfamily of the protein in class %


for x in familyCompleteProteins:
    if x[1] in selectedFamilies:
        proteinName = x[0]
